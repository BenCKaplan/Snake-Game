...
void draw_Gamewindow(gamewindow_t *r) {
	...
	// Draw Top of room
	for (row_counter = r->upper_left_x;row_counter <= (r->upper_left_x + r->width); row_counter++) {
		int top = mvprintw(r->upper_left_y, row_counter, "%c", r->draw_char);
		top;
	}

	// Draw left side of room
	for (column_counter = r->upper_left_y; column_counter <= (r->upper_left_y + r->height); column_counter++) {
